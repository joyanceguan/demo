...
public class ThreadPoolExecutorWrapper extends ThreadPoolExecutor {
	 private static final Logger LOGGER = LoggerFactory.getLogger(ThreadPoolExecutorWrapper.class);
	     ...
	     * 线程池名称 
	     */
	    public ThreadPoolExecutorWrapper(int corePoolSize, int maximumPoolSize, long keepAliveTime, TimeUnit unit, BlockingQueue workQueue,
	                         String poolName) {
	...
	// 统计任务耗时、初始线程数、核心线程数、正在执行的任务数量、已完成任务数量、任务总数、队列里缓存的任务数量、池中存在的最大线程数、最大允许的线程数、线程空闲时间、线程池是否关闭、线程池是否终止 
	    ...
	    public static ExecutorService newCachedThreadPool(String poolName) {
	        return new ThreadPoolExecutorWrapper(0, Integer.MAX_VALUE, 60L, TimeUnit.SECONDS, new SynchronousQueue (), poolName);
	    }
	    /**
	...
	public static void main(String[] args) {
		ExecutorService executorService = ThreadPoolExecutorWrapper.newFixedThreadPool(2, "thread_monitor");
		for(int i=0;i<3;i++){
